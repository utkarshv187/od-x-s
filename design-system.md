...
- **Brand accent — champagne copper** `#B69377` — **reconciled 2026-07-17 against the brand Figma file**, whose primary variable is `Brown/Brown 400 (Primary) = #B69377`. This supersedes the previously-approved Sentinel gold `#B89E55` (kept in tokens as a web-only alias). Primary CTA is the **brushed-metal gradient** centered on `#B69377` with an **ink label** (never white). **One metal CTA and at most one copper rim-glow per screen.** Brand file also confirms: Figtree type ramp (`Display/20EBold`, `Heading/16*`, `Body/14|12*`, `Caption/10*`), Neutral grey ramp, error tint `Red/Red 300 #FFB2B9`, dark textured canvas + copper-sepia 3D illustration + white sheet with 24px top radius (see `reference/brand-figma-1st-screen.png`).
